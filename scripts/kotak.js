...
// URL to download the HTML from Kotak Bank's FD interest rates page
...
async function fetchKotakData() {
    ...
    table.find('tbody tr').each((i, tr) => {
        ...
        if (tenorBucket.indexOf('Fixed Deposits Interest Rates for Domestic/ NRO / NRE effective from') === 0) return;
        if (tenorBucket.indexOf('Maturity Periods - Premature Withdrawal Allowed') === 0) return;
    ...
    if (formattedData.length === 0) throw new Error('No data found for Kotak');
